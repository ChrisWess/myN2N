...
class RunContext(object):
    ...
    def __init__(self, submit_config: submit.SubmitConfig, max_epoch: Any = None):
        ...
        import main
        with open(os.path.join(submit_config.results_dir, "config.txt"), "w") as f:
            filtered_dict = {k: v for k, v in main.__dict__.items() if not k.startswith("_") and not isinstance(v, (types.ModuleType, types.FunctionType, types.LambdaType, submit.SubmitConfig, type))}
        ...
        with open(os.path.join(submit_config.results_dir, "run.txt"), "w") as f:
    ...
    def update(self) -> None:
        ...
        if os.path.exists(os.path.join(self.submit_config.results_dir, "abort.txt")):
            self.should_stop_flag = True
    ...
    def close(self) -> None:
        ...
        if not self.has_closed:
            ...
            with open(os.path.join(self.submit_config.results_dir, "run.txt"), "w") as f:
                pprint.pprint(self.run_txt_data, stream=f, indent=4, width=200, compact=False)
