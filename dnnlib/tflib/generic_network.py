...
class Network:
    ...
    Args:
        ...
        func: The underlying network construction function.
    ...
    def __init__(self, name: str = None, func: Callable = None, **static_kwargs):
    ...
    def __getstate__(self) -> dict:
        """Pickle export to save a snapshot of this network."""
        return {
            "version": 2,
            "name": self.name,
    ...
    def __setstate__(self, state: dict) -> None:
        """Pickle import to load a snapshot of a (usually trained) network."""
        tfutil.assert_tf_initialized()
        self._init_fields()
        ...
        self._build_func = state["build_func"]
    ...
    def copy_vars_from(self, src_net: "Network") -> None:
        """Copy the values of all variables from the given network to this one."""
        names = [name for name in self.vars.keys() if name in src_net.vars]
        tfutil.set_vars(tfutil.run({self.vars[name]: src_net.vars[name] for name in names}))

    def copy_trainables_from(self, src_net: "Network") -> None:
        """Copy the values of all trainable variables from the given network to this one."""
